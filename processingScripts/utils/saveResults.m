function saveResults(saveDir,nFoldEvalDir)
% saveResults Prints results file to saveDir
% Inputs:
%   saveDir -
%   nFoldEvalDir - 
% eg: '/Users/aashish/dev/java/crwdQA/crowdData/nFoldRealResults','/Users/aashish/dev/java/crwdQA/crowdData/nFoldSets'
% ******************************************
    multiclassFolders = {'AdultContent2','HyunCatherines','wsdStandardized'};
...
%         printFile(fidFMeasureCI,fmeasureMatCIL,fmeasureMatCIR,rowName,rowTypePrint,coloumnName,true);
        accuracyMat(:,[1,3,11]) = [];
        fmeasureMat(:,[1,3,11]) = [];
        allAccuracy = [allAccuracy mean(accuracyMat,2)];
        allFMeasure = [allFMeasure mean(fmeasureMat,2)];
    end
    clearvars -except allAccuracy allFMeasure
    save allFA.mat
end
    ...
    for rows = 1:length(rowName)
        if(isempty(matR))
            ...
            for val =  1:length(rowVals)
                ...
                fprintf(fid,'%s',num2str(rowVals(val),'%1.3f'));
